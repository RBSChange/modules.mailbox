	...
	protected function getVirtualChildren($document, $subModelNames, $propertyName)
	{
		if ($document instanceof generic_persistentdocument_folder)
		{
			...
			if (preg_match('/^\d{4}\-\d{2}\-\d{2}$/', $dateLabel, $matches))
			{
				...
				$countQuery = mailbox_MessageService::getInstance()->createQuery()->add(Restrictions::between('creationdate', $startdate, $endate))->setProjection(Projections::rowCount('countItems'));
				$resultCount = $countQuery->find();
				$this->setTotal(intval($resultCount[0]['countItems']));
				$query = mailbox_MessageService::getInstance()->createQuery()->add(Restrictions::between('creationdate', $startdate, $endate))->addOrder(Order::desc('document_creationdate'))->setFirstResult($offset)->setMaxResults($pageSize);
				return $query->find();
			}
		}
